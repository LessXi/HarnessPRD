...
def serialize_sse_event(event: SSEEvent) -> str:
    ...
    前端 readStream 依赖以下事件格式：
      ...
      data: {"event":"review_result","content":"<review_text>","passed":bool,"issues":[...]}
    ...
    if event.event == "chunk":
        ...
        return f"data: {json.dumps({'event': 'review_result', 'content': event.content, 'passed': event.passed, 'issues': event.issues})}\n\n"
