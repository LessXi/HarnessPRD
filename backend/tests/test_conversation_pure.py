...
class TestFormToKwargs:
    ...
    def test_with_form_data(self, mock_form_dict):
        """有表单数据时输出包含产品名称等字段"""
        kwargs = _form_to_kwargs(mock_form_dict)
...
class TestBuildSystemPrompt:
    ...
    def test_contains_role_definition(self, mock_form_dict):
        """提示词包含产品经理角色定义"""
        prompt = _build_system_prompt(mock_form_dict)
        assert isinstance(prompt, str)
        assert len(prompt) > 200
        assert "产品经理" in prompt or "产品合伙人" in prompt

    def test_contains_form_data(self, mock_form_dict):
        """提示词包含完整的表单数据"""
        prompt = _build_system_prompt(mock_form_dict)
        assert "智能简历助手" in prompt
        assert "一键简历解析" in prompt

    def test_contains_topic_map(self, mock_form_dict):
        """提示词包含 5 个话题方向"""
        prompt = _build_system_prompt(mock_form_dict)
    ...
    def test_contains_skip_logic(self, mock_form_dict):
        """提示词包含跳过对话的兜底逻辑"""
        prompt = _build_system_prompt(mock_form_dict)
        assert "跳过" in prompt
...
class TestBuildLcMessages:
    ...
    def test_first_message(self, mock_form_dict):
        """无历史消息时返回 [System, Human]"""
        system = _build_system_prompt(mock_form_dict)
    ...
    def test_icebreaker(self, mock_form_dict):
    ...
    def test_with_history(self, mock_form_dict, mock_history):
    ...
    def test_with_history_no_new_message(self, mock_form_dict, mock_history):
        """不带新消息时仅返回 [System] + 历史"""
        system = _build_system_prompt(mock_form_dict)
    ...
    def test_empty_input(self, mock_form_dict):
        """全空输入返回 [System]"""
        system = _build_system_prompt(mock_form_dict)
        messages = _build_lc_messages(system, [], "")
        assert len(messages) == 1
